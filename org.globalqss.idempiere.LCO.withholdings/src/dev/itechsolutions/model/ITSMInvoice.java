...
public class ITSMInvoice extends LCO_MInvoice {
	...
	public static ITSMInvoice[] getOfVoucherWithholding(MITSVoucherWithholding voucher) {
		...
		POInfo invoiceInfo = POInfo.getPOInfo(voucher.getCtx(), Table_ID);
		...
		try {
			...
			while (rs.next())
				invoices.add(new ITSMInvoice(voucher.getCtx(), rs, voucher.get_TrxName()));
	...
	public int recalcWithholdings() {
		return recalcWithholdings(null);
	}
	...
	public int recalcWithholdings(MITSVoucherWithholding voucher) {
		...
		if (voucher != null)
		{
			where += " AND LCO_WithholdingType_ID = ?";
			params.add(voucher.getLCO_WithholdingType_ID());
		}
		
		// Search withholding types applicable depending on IsSOTrx
		List<X_LCO_WithholdingType> wts = new Query(getCtx(), X_LCO_WithholdingType.Table_Name, where, get_TrxName())
		...
		for (X_LCO_WithholdingType wt : wts)
		{
			int nodel = DB.executeUpdateEx(
					...
					+ " AND (ITS_VoucherWithholding_ID IS NULL OR EXISTS("
						+ "SELECT 1 FROM ITS_VoucherWithholding vwh"
						+ " WHERE vwh.ITS_VoucherWithholding_ID = LCO_InvoiceWithholding.ITS_VoucherWithholding_ID"
						+ " AND vwh.DocStatus IN ('IP', 'DR')"
					+ "))",
					new Object[] { getC_Invoice_ID(), wt.get_ID() },
					get_TrxName());
			...
			for (X_LCO_WithholdingRule wr : wrs)
			{
				...
				if (base != null &&
					...
					iwh.setIsCalcOnInvoice(wc.isCalcOnInvoice());
					iwh.setIsCalcOnPayment(wc.isCalcOnPayment());
					...
					if (voucher != null)
						iwh.setITS_VoucherWithholding_ID(voucher.get_ID());
