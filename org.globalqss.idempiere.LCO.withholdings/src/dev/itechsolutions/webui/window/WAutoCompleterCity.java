...
public class WAutoCompleterCity extends AutoComplete implements EventListener<Event>
{
	...
	@Override
	public void changeValue(String value, boolean isChangingBySelectBack) {
		showPopupDelayed();
		refreshData(value);
		super.changeValue(value, isChangingBySelectBack);
	}
	...
	public void setC_City_ID(int C_City_ID) {
		...
		Optional<CityVO> city = m_citiesShow
				.stream()
				.filter(cy -> cy.C_City_ID == C_City_ID)
				.findFirst();
		
		if (city.isPresent() && !city.get().equals(ITEM_More))
		{
			setCity(city.get());
	...
	public void onEvent(Event event) throws Exception 
	{
		...
		//event.toString();
		//Argenis Rodríguez - iTech Solutions - Add Events in Auto Complete City Component
		if (Events.ON_SELECT.equals(event.getName()))
		{
			...
			if (index>=0)
			{
				//Argenis Rodríguez - iTech Solutions - Add Support for get City Name from Ordered Items
				String cityName = getItems().get(index).getLabel();
				
				Optional<CityVO> city = m_citiesShow
						.stream()
						.filter(cy -> cy.CityName.equals(cityName))
						.findFirst();
				
				if(event == null || city.isEmpty() || city.get().equals(ITEM_More))
				{
					setCity(null);
					if (city.isEmpty())
						this.setText("");
