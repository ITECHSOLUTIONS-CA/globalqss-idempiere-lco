...
package dev.itechsolutions.webui.window;
...
import dev.itechsolutions.model.MITSLocation;
 ...
 * 				 - http://jira.idempiere.com/browse/IDEMPIERE-147 <br/>
 * 				 - Argenis Rodríguez / iTechSolutions C.A.
 * 				 - Add support for Venezuela Location
 **/
public class WLocationDialogExt extends Window implements EventListener<Event>
{
	/**
	 * 
	 */
	...
	/** Logger          */
	private static final CLogger log = CLogger.getCLogger(WLocationDialogExt.class);
	...
	//Added Fields to Venezuela Location
	...
	private MITSLocation   m_location;
	private int         m_origCountry_ID;
	private int         s_oldCountry_ID = 0;
	private int m_origRegion_ID = 0;
	private int s_oldRegion_ID = 0;
	private int m_origMunicipality_ID = 0;
	...
	//Added fields for Venezuela Location
	...
	public WLocationDialogExt(String title, MITSLocation location)
	{
		this (title, location, null);
	}
	
	public WLocationDialogExt(String title, MITSLocation location, GridField gridField) {
		m_GridField  = gridField;
		m_location = location;
		if (m_location == null)
			m_location = new MITSLocation (Env.getCtx(), 0, null);
		//  Overwrite title 
		...
		for (MMunicipality municipality: MMunicipality.getMunicipalities(m_origRegion_ID))
		{
		...
		for (MParish parish: MParish.getParishes(m_origMunicipality_ID))
		{
			lstParish.appendItem(parish.getName(), parish);
		}
		setParish();
		
		//Init Location First Time
		initLocation();
		//
	...
	private void initComponents()
	{
		...
		lblMunicipality.setClass("field-label");
		lblParish = new Label(Msg.getMsg(Env.getCtx(), "Parish"));
		lblParish.setClass("field-label");
		
		txtAddress1 = new Textbox();
		txtAddress1.setCols(20);
		txtAddress1.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Address1));
		txtAddress2 = new Textbox();
		txtAddress2.setCols(20);
		txtAddress2.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Address2));
		txtAddress3 = new Textbox();
		txtAddress3.setCols(20);
		txtAddress3.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Address3));
		txtAddress4 = new Textbox();
		txtAddress4.setCols(20);
		txtAddress4.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Address4));
		txtAddress5 = new Textbox();
		txtAddress5.setCols(20);
		txtAddress5.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Address5));
		txtComments = new Textbox();
		txtComments.setCols(20);
		txtComments.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Comments));
		
		//autocomplete City
		txtCity = new WAutoCompleterCity(m_WindowNo);
		txtCity.setCols(20);
		txtCity.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_City));
		...
		txtPostal.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Postal));
		txtPostalAdd = new Textbox();
		txtPostalAdd.setCols(20);
		txtPostalAdd.setMaxlength(MITSLocation.getFieldLength(MITSLocation.COLUMNNAME_Postal_Add));
		...
		lstParish.setRows(0);
		//End of fields for Venezuela Location
	...
	private void init()
	{
		...
		if (MITSLocation.LOCATION_MAPS_URL_PREFIX == null)
			toLink.setVisible(false);
		pnlLinks.appendChild(toRoute);
		if (MITSLocation.LOCATION_MAPS_ROUTE_PREFIX == null || Env.getAD_Org_ID(Env.getCtx()) <= 0)
		...
		if (MITSLocation.LOCATION_MAPS_URL_PREFIX != null || MITSLocation.LOCATION_MAPS_ROUTE_PREFIX != null)
	...
	private void initLocation()
	{
		...
		if (m_location.getC_Region_ID() != s_oldRegion_ID)
		{
			...
			txtCity.fillList();
			txtCity.refresh("");
			if (m_location.isHasMunicipality())
			{
				lstMunicipality.appendItem("", null);
				for (MMunicipality municipality: MMunicipality.getMunicipalities(m_location.getC_Region_ID()))
					lstMunicipality.appendItem(municipality.getName(), municipality);
			}
			s_oldRegion_ID = m_location.getC_Region_ID();
		}
		
		if (m_location.getC_Municipality_ID() > 0 && region != null
				&& m_location.getMunicipality().getC_Region_ID() == region.get_ID())
			setMunicipality();
		else
		{
		...
		if (m_location.isHasMunicipality() && m_location.getC_Municipality_ID() > 0)
			Env.setContext(Env.getCtx(), m_WindowNo, Env.TAB_INFO, "C_Municipality_ID", m_location.getC_Municipality_ID());
		else
		...
		if (m_location.getC_Municipality_ID() != s_oldMunicipality_ID)
		{
			...
			if (m_location.isHasParish())
			{
				lstParish.appendItem("", null);
				for (MParish parish: MParish.getParishes(m_location.getC_Municipality_ID()))
					lstParish.appendItem(parish.getName(), parish);
			}
			s_oldMunicipality_ID = m_location.getC_Municipality_ID();
		}
		
		if (m_location.getC_Parish_ID() > 0 && municipality != null
				&& m_location.getParish().getC_Municipality_ID() == municipality.get_ID())
			setParish();
		else
		{
		...
		if (m_location.isHasParish() && m_location.getC_Parish_ID() > 0)
			Env.setContext(Env.getCtx(), m_WindowNo, Env.TAB_INFO, "C_Parish_ID", m_location.getC_Parish_ID());
		else
			Env.setContext(Env.getCtx(), m_WindowNo, Env.TAB_INFO, "C_Parish_ID", 0);
	...
	private void setMunicipality() {
		if (m_location.getMunicipality() != null)
		{
			...
			while (iter.hasNext())
			{
				...
				if (m_location.getMunicipality().equals(item.getValue()))
					lstMunicipality.setSelectedItem(item);
			}
		}
		else
			lstMunicipality.setSelectedItem(null);
	...
	private void setParish() {
		...
		{
			List<?> listParish = lstParish.getChildren();
			Iterator<?> iter = listParish.iterator();
			
			while (iter.hasNext())
			{
				ListItem item = (ListItem) iter.next();
				if (m_location.getParish().equals(item.getValue()))
					lstParish.setSelectedItem(item);
			}
		}
		else
			lstParish.setSelectedItem(null);
	...
	/**
	 *  Get edited Value (MITSLocation)
	 *  @return location
	 */
	public MITSLocation getValue()
	{
		return m_location;
	}   
	...
	public void onEvent(Event event) throws Exception
	{
			...
			if (msg != null) {
				...
				Dialog.error(0, "FillMandatory", Msg.parseTranslation(Env.getCtx(), msg), new Callback<Integer>() {					
					...
					public void onCallback(Integer result) {
						Events.echoEvent("onSaveError", WLocationDialogExt.this, null);
					}
				});
				inOKAction = false;
				return;
		...
		{
			String urlString = MITSLocation.LOCATION_MAPS_URL_PREFIX + getFullAdress();
			String message = null;
			try {
		...
		else if (toRoute.equals(event.getTarget()))
		{
			...
			if (AD_Org_ID != 0){
				...
				MITSLocation orgLocation = new MITSLocation(Env.getCtx(),orgInfo.getC_Location_ID(),null);

				String urlString = MITSLocation.LOCATION_MAPS_ROUTE_PREFIX +
						         MITSLocation.LOCATION_MAPS_SOURCE_ADDRESS + orgLocation.getMapsLocation() + //org
						         MITSLocation.LOCATION_MAPS_DESTINATION_ADDRESS + getFullAdress(); //partner
				String message = null;
				try {
					Executions.getCurrent().sendRedirect(urlString, "_blank");
				}
		...
		else if (btnOnline.equals(event.getTarget()))
		{
			...
			MITSLocation m_location = new MITSLocation(Env.getCtx(), 0, null);
	...
	private boolean action_OK()
	{
		...
		if (country.get_ValueAsBoolean(ColumnUtils.COLUMNNAME_HasMunicipality)
				&& lstMunicipality.getSelectedItem() != null)
		{
			MMunicipality m = (MMunicipality)lstMunicipality.getSelectedItem().getValue();
			m_location.setMunicipality(m);
		}
		...
		if (country.get_ValueAsBoolean(ColumnUtils.COLUMNNAME_HasParish)
				&& lstParish.getSelectedItem() != null)
		{
			MParish p = (MParish)lstParish.getSelectedItem().getValue();
			m_location.setParish(p);
		}
		...
		boolean changedCity = m_location.is_ValueChanged(MITSLocation.COLUMNNAME_City) || m_location.is_ValueChanged(MITSLocation.COLUMNNAME_C_City_ID);
		boolean changedAddress1 = m_location.is_ValueChanged(MITSLocation.COLUMNNAME_Address1);
		boolean changedAddress2 = m_location.is_ValueChanged(MITSLocation.COLUMNNAME_Address2);
		boolean changedRegion = m_location.is_ValueChanged(MITSLocation.COLUMNNAME_RegionName) || m_location.is_ValueChanged(MITSLocation.COLUMNNAME_C_Region_ID);
		//  Save changes
		...
		if (success) {
			...
			if (   changedCity
	        		...
	        		&& !m_GridField.getGridTab().getValueAsBoolean("IsPreserveCustomName"))
	    		{
	    			...
	    			int bplID = DB.getSQLValueEx(trx.getTrxName(), MITSLocation.updateBPLocName, m_location.getC_Location_ID());
	    			if (bplID>0)
	    			{
	...
	public void dispose()
	{
		...
		{
			m_location = new MITSLocation(m_location.getCtx(), m_location.get_ID(), null);
		}	
		super.dispose();
	}
