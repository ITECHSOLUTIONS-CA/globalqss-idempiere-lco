...
import dev.itechsolutions.webui.window.WAutoCompleterCity;
...
public class WLocationExtDialog extends Window implements EventListener<Event>
{
	...
	private ITSMLocation   m_location;
	...
	public WLocationExtDialog(String title, ITSMLocation location)
	{
		this (title, location, null);
	}

	public WLocationExtDialog(String title, ITSMLocation location, GridField gridField) {
		m_GridField  = gridField;
		m_location = location;
		if (m_location == null)
			m_location = new ITSMLocation(Env.getCtx(), 0, null);
		//  Overwrite title 
		...
		for (MMunicipality municipality : MMunicipality.getMunicipalities(m_location.getRegion()))
		{
			lstMunicipality.appendItem(municipality.getName(),municipality);
		}
		setMunicipality();
		// Parish
		m_origMunicipality_ID =  m_location.get_ValueAsInt(MMunicipality.COLUMNNAME_C_Municipality_ID);
		m_selectedMunicipality_ID = m_origMunicipality_ID;	// lstMunicipality.getSelectedCount();	
		//
		lstParish.appendItem("", null); 
		for (MParish parish : MParish.getParishes(m_selectedMunicipality_ID))
		{
	...
	private void initComponents()
	{
		...
		txtAddress1.setMaxlength(ITSMLocation.getFieldLength(ITSMLocation.COLUMNNAME_Address1));
		txtAddress2 = new Textbox();
		txtAddress2.setCols(20);
		txtAddress2.setMaxlength(ITSMLocation.getFieldLength(ITSMLocation.COLUMNNAME_Address2));
		txtAddress3 = new Textbox();
		txtAddress3.setCols(20);
		txtAddress3.setMaxlength(ITSMLocation.getFieldLength(ITSMLocation.COLUMNNAME_Address3));
		...
		txtPostal.setMaxlength(ITSMLocation.getFieldLength(ITSMLocation.COLUMNNAME_Postal));
		txtPostalAdd = new Textbox();
		txtPostalAdd.setCols(20);
		txtPostalAdd.setMaxlength(ITSMLocation.getFieldLength(ITSMLocation.COLUMNNAME_Postal_Add));
	...
	private void init()
	{
		...
		if (ITSMLocation.LOCATION_MAPS_URL_PREFIX == null)
			toLink.setVisible(false);
		pnlLinks.appendChild(toRoute);
		if (ITSMLocation.LOCATION_MAPS_ROUTE_PREFIX == null || Env.getAD_Org_ID(Env.getCtx()) <= 0)
		...
		if (ITSMLocation.LOCATION_MAPS_URL_PREFIX != null || ITSMLocation.LOCATION_MAPS_ROUTE_PREFIX != null)
...
//			log.warning("getC_Municipality_ID()"+m_location.getC_Municipality_ID());		
		...
		if (m_location.get_ValueAsInt(MMunicipality.COLUMNNAME_C_Municipality_ID) != s_oldMunicipality_ID)
		{
			// Clear Parish
//			setParish();
			...
			lstParish.getChildren().clear();
			lstParish.appendItem("", null); 
			for (MParish parish : MParish.getParishes(m_location.get_ValueAsInt(MMunicipality.COLUMNNAME_C_Municipality_ID)))
			{
				lstParish.appendItem(parish.getName(),parish);
			}
			setParish();
			s_oldMunicipality_ID = m_location.get_ValueAsInt(MMunicipality.COLUMNNAME_C_Municipality_ID);
		}
		//  new Parish
		...
		while (st.hasMoreTokens())
		{
			...
			}else if (s.startsWith("PA") && s.trim().equalsIgnoreCase("PA")
					&& m_location.isHasRegion() && m_location.isHasParish()) {
			...
			} else if (s.startsWith("R") && m_location.isHasRegion()) {
				addComponents((Row)lstRegion.getParent());
				isRegionMandatory = s.endsWith("!");
			}
			// NEW FIELDS ELEMENTS
			if (m_location.isHasRegion()) {
	...
	*/
	...
	public void onEvent(Event event) throws Exception
	{
		if (event.getTarget() == confirmPanel.getButton(ConfirmPanel.A_OK)) 
		{
			...
			if (m_location.isHasRegion() && lstRegion.getSelectedItem() == null) {
		...
		{
			String urlString = ITSMLocation.LOCATION_MAPS_URL_PREFIX + getFullAdress();
			String message = null;
			try {
		...
		else if (toRoute.equals(event.getTarget()))
		{
			...
			if (AD_Org_ID != 0){
				...
				ITSMLocation orgLocation = new ITSMLocation(Env.getCtx(),orgInfo.getC_Location_ID(),null);
				
				String urlString = ITSMLocation.LOCATION_MAPS_ROUTE_PREFIX +
						         ITSMLocation.LOCATION_MAPS_SOURCE_ADDRESS + orgLocation.getMapsLocation() + //org
						         ITSMLocation.LOCATION_MAPS_DESTINATION_ADDRESS + getFullAdress(); //partner
				String message = null;
				try {
					Executions.getCurrent().sendRedirect(urlString, "_blank");
				}
		...
		else if (btnOnline.equals(event.getTarget()))
		{
			...
			ITSMLocation m_location = new ITSMLocation(Env.getCtx(), 0, null);
		...
		else if (event.getTarget().equals(txtCity))
		{
			...
			int C_City_ID = txtCity.getC_City_ID();
			...
			MCity city = MCity.get(C_City_ID);
...
//    				bpl.setName(bpl.getBPLocName(m_location));
	...
	public void dispose()
	{
		...
		{
			m_location = new ITSMLocation(m_location.getCtx(), m_location.get_ID(), null);
		}
		super.dispose();
	}
